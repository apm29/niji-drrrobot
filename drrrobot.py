...
class Bot(object):
    ...
    def tips(self):
        while 1:
            ...
            elif 20 < int(time.strftime('%H', time.localtime(time.time()))) < 24:
                list_tips = ['!!!!!!!!!!!!!',
                             '夜都睡了，妳還不睡嗎？',
                             '晚安，月亮也壹起睡了',
                             '夜深了，天上的星星都已經睡了',
    ...
    def feedback(self, message, name_sender, to=''):
        if re.findall('/feedback .*', message):
            ...
            try:
                ...
                mail['Subject'] = '使用者反馈'
                mail['From'] = email.utils.formataddr(['ReRe', 'niji_drrrobot@126.com'])
                ...
                server.sendmail(from_addr='niji_drrrobot@126.com', to_addrs='willtimecondense@qq.com',
                                msg=mail.as_string())
                server.quit()
    ...
    def help(self, to=''):
        self.post(message='本萌妹的指令詳見鏈接', url='https://drrr.wiki/%E8%BC%95%E9%A3%9F%E5%92%96%E5%95%A1%E9%A4%A8#BOT',
                  to=to)
